...
func init() {
	...
	profileCreateCmd.Flags().StringVar(&specFile, "spec-file", "", "Path to YAML file containing the Profile spec (required)")
	profileCreateCmd.Flags().StringVar(&resourceName, "name", "aws-us-east-1", "Name of the Profile resource to create/update")
...
var profileCreateCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err := createOrUpdateProfile(cmd.Context(), dyn, u); err != nil {
			return fmt.Errorf("create/update Profile %s: %w", u.GetName(), err)
		}
...
func createOrUpdateProfile(ctx context.Context, dyn dynamic.Interface, u *unstructured.Unstructured) error {
